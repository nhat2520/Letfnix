...
def create_and_save_tfidf_matrix(documents, filename):
...
def calculate_and_save_cosine_similarity(userId, tfidf_matrix_filename):
    ...
    save_npz(f'cosine_sim_{userId}.npz', csr_matrix(cosine_sim))

# Function to get recommendations based on a userId
def get_recommendations(userId, tfidf_matrix_filename):
    cosine_sim = load_npz(f'cosine_sim_{userId}.npz').toarray()
...
def update_user_and_cosine_similarity(userId, movie_idx, action, tfidf_matrix_filename):
    ...
    with open(f'user_profile_{userId}.pkl', 'wb') as f:
        pickle.dump(user_profile, f)
    user_profile = user_profile.reshape(1, -1)
    updated_cosine_sim = cosine_similarity(user_profile, tfidf_matrix)
    save_npz(f'cosine_sim_{userId}.npz', csr_matrix(updated_cosine_sim))

data = pd.read_csv('recommend\data.csv')
ratings = pd.read_csv('recommend\shortened_ratings.csv')
create_and_save_tfidf_matrix(data['description'],'v1')
create_and_save_user_profile(1, 'v1')
calculate_and_save_cosine_similarity(1, 'v1')
print(data.loc[get_recommendations(1, 'v1')]['original_title'])
update_user_and_cosine_similarity(1, 1, 'view', 'v1')
update_user_and_cosine_similarity(1, 2, 'buy', 'v1')
print(data.loc[get_recommendations(1, 'v1')]['original_title'])
