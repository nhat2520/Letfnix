...
def create_and_save_tfidf_matrix_v1():
    ...
    with open("repo/tfidf_v1.pkl", "wb") as f:
        pickle.dump(tf, f)
    save_npz("repo/tfidf_matrix_v1.npz", tfidf_matrix)
...
def create_and_save_user_profile(user_id, tfidf_matrix_filename):
    ...
    with open(f"repo/user_profile_{user_id}.pkl", "wb") as f:
...
def calculate_and_save_similarity_matrix(user_id, tfidf_matrix_filename):
    ...
    save_npz(f"repo/sim_matrix_{user_id}.npz", csr_matrix(cosine_sim))
...
def get_recommendations(user_id, length):
    ...
    cosine_sim = load_npz(f"repo/sim_matrix_{user_id}.npz").toarray()
    top_n_indices = cosine_sim[0].argsort()[-length:-1][::-1]
    data = pd.read_csv('recommend/data.csv')
...
def update_user_and_similarity_matrix(
    ...
    user_profile = update_user_profile(user_profile, movie_tfidf_vector,
                                       action)
    ...
    save_npz(f"server/repo/sim_matrix_{user_id}.npz",
             csr_matrix(updated_cosine_sim))
